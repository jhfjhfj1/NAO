...
def train():
    ...
    while True:
        ...
        if Params.arch_pool is None:
            arch_pool = utils.generate_arch(Params.num_seed_arch, Params.num_cells,
                                            5)  # [[[conv],[reduc]]]
            ...
            if Params.sample_policy == 'uniform':
                ...
                encoder_input = list(map(lambda x: utils.parse_arch_to_seq(x[0], branch_length) + \
                                                   utils.parse_arch_to_seq(x[1], branch_length),
                                         Params.arch_pool))
                predicted_error_rate = controller.predict(encoder_input)
        ...
        new_arch_pool = old_archs[:len(old_archs) - (num_new_archs + int(new_arch_lower_bound / 10))] + new_archs + utils.generate_arch(int(new_arch_lower_bound / 10), 5, 5)
